...
def GAM_fit(gen, disc, comp, norm_func_img, norm_func_latent, dataloader, lrs = [ 0.0002, 0.9, 0.999],  
    ...
    if enc is not None:
        data_latents = norm_func_latent(enc(norm_func_img(data_imgs, mean_tensor, std_tensor))).reshape((-1, 197, 1, 1)).to(device)
    else :
    ...
    for epoch in range(num_epochs):
        ...
        for X, y in dataloader:
            ...
            if enc is None:
                ...
                y = norm_func_latent(enc(norm_func_img(X, mean_tensor, std_tensor))).reshape((-1, 197, 1, 1)).to(device)
